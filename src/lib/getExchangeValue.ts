...
export async function getExchangeValue(
  ...
  for (const [currency, walletAddress] of Object.entries(wallets)) {
    try {
      ...
      const exchanges = allRates.filter(
        ({ exchangeRate }) =>
      ...
      const averageRate =
        exchanges.reduce((sum, exchange) => sum + exchange.exchangeRate!, 0) /
        exchanges.length;
      ...
      console.error(
        `Failed to process ${currency} wallet ${walletAddress}:`,
        error,
      );
      result[currency] = {
        wallet: walletAddress,
  ...
  const results = await Promise.all(
    exchanges.map(async (exchangeName) => {
      try {
        ...
        console.error(
          `Failed to get rate from ${exchangeName} for ${currency}:`,
          error,
        );
        return null;
      }
...
async function getBitcoinBalance(address: string): Promise<number> {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      console.error(
        `Error fetching Bitcoin balance from ${endpoint.url}:`,
        error,
      );
  ...
  throw new Error(
    `Failed to get Bitcoin balance: ${lastError?.message || "Unknown error"}`,
  );
}
...
async function getEthereumBalance(address: string): Promise<number> {
  ...
  throw new Error(
    `Failed to get Ethereum balance: ${lastError?.message || "Unknown error"}`,
  );
}
...
async function getExchangeRate(
  ...
  try {
    ...
    throw new Error(
      `Failed to get exchange rate: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
